...
PairTableZ::PairTableZ(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
void PairTableZ::compute(int eflag, int vflag)
{
  ...
  double maxZ, zz, dfdz, deltaFz, scalingZ;		// SM
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
		...
		// no decay interaction if both atoms are below threshold1
		if (x[j][2]<=threshold1 && ztmp<=threshold1) { 	  
			scalingZ = a0;		
			deltaFz = 0.0;
		}
		
		// if atleast one atom is above threshold2:
		//	- no decay force
		...
		// if atleast one atom is located in between thresholds (and not above threshold2):
		//  - decay force  = -df/dz
        ...
        if (tabstyle == LOOKUP) {
		  ...
		  deltaFz *= (tb->e[itable] + fraction*tb->de[itable]);	//SM
        } 
      ...
      ((a*a*a-a)*tb->e2[itable] + (b*b*b-b)*tb->e2[itable+1]) * tb->deltasq6);		// SM
        } 
		else {
          rsq_lookup.f = rsq;
...
void PairTableZ::settings(int narg, char **arg)
{
...
void PairTableZ::coeff(int narg, char **arg)
{
  if (narg != 8 && narg != 9) error->all(FLERR,"Illegal pair_coeff command"); // SM
  ...
  if (narg == 9) tb->cut = force->numeric(FLERR,arg[8]);	// SM
  else if (tb->rflag) tb->cut = tb->rhi;
  else tb->cut = tb->rfile[tb->ninput-1];

  // SM: set threshold1, threshold2, a0, a1 for decay
  threshold1 = force->numeric(FLERR,arg[4]);
  threshold2 = force->numeric(FLERR,arg[5]);
  a0 = force->numeric(FLERR,arg[6]);
  a1 = force->numeric(FLERR,arg[7]);
...
double PairTableZ::init_one(int i, int j)
{
...
void PairTableZ::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairTableZ::bcast_table(Table *tb)
{
...
void PairTableZ::spline_table(Table *tb)
{
...
void PairTableZ::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTableZ::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairTableZ::null_table(Table *tb)
{
...
void PairTableZ::free_table(Table *tb)
{
...
void PairTableZ::spline(double *x, double *y, int n,
...
double PairTableZ::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairTableZ::write_restart(FILE *fp)
{
...
void PairTableZ::read_restart(FILE *fp)
{
...
void PairTableZ::write_restart_settings(FILE *fp)
{
...
void PairTableZ::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTableZ::single(int i, int j, int itype, int jtype, double rsq,
                         ...
                         double &fforce)
{
  ...
  // SM: define polynomial decay for particle(s) within thresholds
  double **x = atom->x;
  double maxZ, zz, scalingZ;

  // no decay interaction if both atoms are below threshold1
  if (x[j][2]<=threshold1 && x[i][2]<=threshold1) { 	  
	scalingZ = a0;		
  }

  // if atleast one atom is above threshold2:
  //  - set decay potential to the value at the upper threshold
  else if (x[j][2]>=threshold2 || x[i][2]>=threshold2) { 
	zz = threshold2 - threshold1;	
	scalingZ = a0 + a1*zz*zz;
  }
  ...
  //  - decay potential = a0 + a1*zz^2
	...
	if (x[i][2] >= x[j][2]) maxZ = x[i][2];
	if (x[i][2] < x[j][2]) maxZ = x[j][2];	
	zz = maxZ - threshold1;		
	scalingZ = a0 + a1*zz*zz;
  }	
  // -------------------------------------------------------------
  ...
  return factor_lj*scalingZ*phi;	// SM
}
...
void *PairTableZ::extract(const char *str, int &dim)
{
