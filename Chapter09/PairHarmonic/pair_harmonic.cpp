...
PairHarmonic::~PairHarmonic()
{
  if (allocated) {
    ...
    memory->destroy(ksp);	// SM
    memory->destroy(e0);	// SM
    memory->destroy(r0);
...
void PairHarmonic::allocate()
{
  ...
  memory->create(ksp,n+1,n+1,"pair:ksp");	// SM
  memory->create(e0,n+1,n+1,"pair:e0");		// SM
...
void PairHarmonic::coeff(int narg, char **arg)
{
  ...
  double ksp_one = force->numeric(FLERR,arg[2]);	// SM
  double r0_one = force->numeric(FLERR,arg[3]);		// SM
  double e0_one = force->numeric(FLERR,arg[4]);		// SM
...
double PairHarmonic::init_one(int i, int j)
{
  ...
  if (offset_flag) {
	double dr = cut[i][j] - r0[i][j];						// SM
    offset[i][j] = 0.5 * ksp[i][j] * (dr*dr) - e0[i][j];	// SM
  } else offset[i][j] = 0.0;
...
void PairHarmonic::write_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        fwrite(&ksp[i][j],sizeof(double),1,fp);	// SM
        fwrite(&e0[i][j],sizeof(double),1,fp);	// SM
...
void PairHarmonic::read_restart(FILE *fp)
{
  ...
  for (i = 1; i <= atom->ntypes; i++)
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          utils::sfread(FLERR,&ksp[i][j],sizeof(double),1,fp,NULL,error);	// SM
          utils::sfread(FLERR,&e0[i][j],sizeof(double),1,fp,NULL,error);	// SM
          utils::sfread(FLERR,&r0[i][j],sizeof(double),1,fp,NULL,error);
          utils::sfread(FLERR,&cut[i][j],sizeof(double),1,fp,NULL,error);
        }
        MPI_Bcast(&ksp[i][j],1,MPI_DOUBLE,0,world);	// SM
        MPI_Bcast(&e0[i][j],1,MPI_DOUBLE,0,world);	// SM
  ...
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp,"%d %d %g %g %g %g\n",
              i,j,ksp[i][j],e0[i][j],r0[i][j],cut[i][j]);	// SM
}
                         ...
                         double &fforce)
{
  ...
  r = sqrt(rsq);
  dr = r - r0[itype][jtype];
  fforce = -factor_lj * ksp[itype][jtype] * dr / r;	// SM

  phi = 0.5 * ksp[itype][jtype] * (dr*dr) - e0[itype][jtype] - offset[itype][jtype];	// SM
...
{
  dim = 2;
  if (strcmp(str,"ksp") == 0) return (void *) ksp;	// SM
  if (strcmp(str,"r0") == 0) return (void *) r0;
  if (strcmp(str,"e0") == 0) return (void *) e0;	// SM
  return NULL;
}
