...
#include "fix_widom_modified.h"	// JL
  ...
  random_equal(nullptr)
{
  ...
  nevery = force->inumeric(FLERR,arg[3]);				// JL
  ninsertions = force->inumeric(FLERR,arg[4]);			// JL
  nwidom_type = force->inumeric(FLERR,arg[5]);			// JL
  seed = force->inumeric(FLERR,arg[6]);					// JL
  insertion_temperature = force->numeric(FLERR,arg[7]);	// JL
  ...
  if (exchmode == EXCHMOL) {
    ...
    if (atom->molecular == 2 && onemols != atom->avec->onemols)	// JL
...
void FixWidom::options(int narg, char **arg)
{
  ...
  if (strcmp(arg[iarg],"mol") == 0) {
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix widom command");
      charge = force->numeric(FLERR,arg[iarg+1]);	// JL
      charge_flag = true;
      iarg += 2;
...
void FixWidom::init()
{
  ...
  char **group_arg = new char*[4];	// JL
  
  if (full_flag && !exclusion_group_bit) {
    // create unique group name for atoms to be excluded

	// JL
    int len = strlen(id) + 29;
    group_arg[0] = new char[len];
    sprintf(group_arg[0], "FixWidom:widom_exclusion_group:%s", id);
    group_arg[1] = (char *) "substract all all";
    ...
    arg[0] = (char *) "exclude";
    arg[1] = (char *) "group";
    arg[2] = group_arg[0];					// JL
    // arg[2] = (char *) group_id.c_str();	// JL
    arg[3] = (char *) "all";
  ...
  if (exchmode == EXCHMOL) {
    ...
    int len = strlen(id) + 29;
    group_arg[0] = new char[len];
    sprintf(group_arg[0], "FixWidom:rotation_gas_atoms:%s", id);
    group_arg[1] = (char *) "molecule -1";
    group->assign(2, group_arg);
...
void FixWidom::attempt_atomic_insertion_full()
{
  ...
  for (int imove = 0; imove < ninsertions; imove++) {
    ...
    if (atom->tag_enable) {
      atom->tag_extend();
      if (atom->map_style != 0) atom->map_init();	// JL
    }
    atom->nghost = 0;
    if (triclinic) domain->x2lamda(atom->nlocal);
    comm->borders();
