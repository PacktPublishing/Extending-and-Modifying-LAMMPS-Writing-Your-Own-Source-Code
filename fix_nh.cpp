...
FixNH::FixNH(LAMMPS *lmp, int narg, char **arg) :
  ...
  etap_mass(NULL), boostStr(NULL)	// SM
{
  ...
  while (iarg < narg) {
	...
	// SM: tempExp option to increment timesteps exponentially
    if (strcmp(arg[iarg],"tempExp") == 0) {
      if (iarg+6 > narg) error->all(FLERR,"Illegal fix nvt/npt/nph command");
      tstat_flag = 1;      
	  tempExpFlag = 1;
	  
      t_start = force->numeric(FLERR,arg[iarg+1]);
	  t_period = force->numeric(FLERR,arg[iarg+2]);
	  betaTemp = force->numeric(FLERR,arg[iarg+3]);
      t_stop = t_start;
	  	  
	  if (strstr(arg[iarg+4],"v_") == arg[iarg+4]) {
		int n = strlen(&arg[iarg+4][2]) + 1;
		boostStr = new char[n];
		strcpy(boostStr,&arg[iarg+4][2]);
	  } else {
		boostV = force->numeric(FLERR,arg[iarg+4]);
		boostStyle = CONSTANT;
	  }
	  
      tempExpScalar = force->inumeric(FLERR,arg[iarg+5]);
      if ((tempExpScalar != 0) && (tempExpScalar != 1)) 
		error->all(FLERR,"tempExpScalar for fix tempExp must be 0 or 1");
...
void FixNH::init()
{
  // SM: check variable names
  if (boostStr) {
    boostVar = input->variable->find(boostStr);
    if (boostVar < 0)
      error->all(FLERR,"Variable name for fix tempExp does not exist");
    if (input->variable->equalstyle(boostVar)) boostStyle = EQUAL;
    else error->all(FLERR,"Variable for fix tempExp is invalid style");
  }
...
double FixNH::compute_scalar()
{
  ...
  // SM: return t_target or energy
  if (tempExpScalar == 1) return t_target;
  else return energy;
}
