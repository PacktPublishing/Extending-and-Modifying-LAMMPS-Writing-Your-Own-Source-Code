...
enum{LJ93,LJ126,LJ1043,COLLOID,HARMONIC,MORSE,LJ126Expanded};	// SM
  ...
  idregion(NULL)
{
  ...
  else if (strcmp(arg[4],"lj126Expanded") == 0) style = LJ126Expanded;	// SM
  ...
  // SM
    ...
    deltaLJ = force->numeric(FLERR,arg[7]);
    cutoff = force->numeric(FLERR,arg[8]);
  }
  ...
  for (i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      for (m = 0; m < n; m++) {
		...
		else if (style == LJ126Expanded) lj126Expanded(region->contact[m].r);	// SM
...
/* ----------------------------------------------------------------------
   SM: LJ 12/6 Expanded interaction for particle with wall
   compute eng and fwall = magnitude of wall force
------------------------------------------------------------------------- */
